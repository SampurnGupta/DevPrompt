...
- [ ] **0.1 — Strengthen `minimum_required_tools` for debug tasks**
...
- [ ] **0.2 — Remove `run_tests` from `minimum_required_tools` for test-writing tasks**
...
- [ ] **0.3 — Strengthen `minimum_required_tools` for scaffold tasks**
  - **File:** `src/gold_standards_part3.py`
...
- [ ] **0.4 — Strengthen `minimum_required_tools` for composite tasks**
  - **File:** `src/gold_standards_part3.py`
...
- [ ] **0.5 — Add `alternative_tool_sequences` to all 48 tasks** *(most impactful)*
...
- [ ] **0.6 — Extend `intent_preservation_reference` for all 48 tasks**
  - **Files:** All three `gold_standards_part*.py`
...
- [ ] **0.7 — Update `compute_agentic_metrics()` to score against best-matching sequence**
...
- [ ] **0.8 — Update `build_tasks_master.py` to propagate `alternative_tool_sequences`**
...
- [ ] **0.9 — Rebuild `tasks_master.json` and re-score tool metrics**
...
- [ ] **1.1 — Fix statistical test: `ttest_ind` → `ttest_rel`**
...
- [ ] **1.2 — Consolidate Cohen's d to one implementation**
...
- [ ] **1.3 — Add Bonferroni correction and corrected α**
...
- [ ] **1.4 — Add 95% CIs to all reported means**
  - **File:** `src/5_analyze_results.py`
...
- [ ] **1.5 — Fix "skip already scored" bug in `4_score_results.py`**
...
- [ ] **1.6 — Add rescore modes to `4_score_results.py`**
...
- [ ] **1.7 — Fix Condition A label in `3_run_experiment.py`**
...
- [ ] **1.8 — Fix `api_clients.py` header comment**
...
- [ ] **1.9 — Fix figure docstring in `6_generate_figures.py`**
  - **File:** `src/6_generate_figures.py` lines 3-12
  - **Bug:** Docstring says "6 main figures" and lists only 6 — actual output is 13 figures
  - **Fix:** Update docstring to list all 13 figures correctly
...
- [ ] **3.1 — Add few-shot TOOL_CALL examples to `TOOL_SYSTEM_PROMPT`**
...
- [ ] **3.2 — Implement `judge_intent_fidelity()` in `4_score_results.py`**
...
- [ ] **3.3 — Write unit test files for applicable tasks (~20 files)**
  - Directory: `tests/`
...
- [ ] **3.4 — Implement Pass@1 scoring in `4_score_results.py`**
...
- [ ] **3.5 — Add Bootstrap CI on CPSTC to `5_analyze_results.py`**
...
- [ ] **4.1 — Compute Format Compliance Rate from existing data**
...
- [ ] **4.2 — Compute Tool Hallucination Rate from existing data**
...
- [ ] **4.3 — Add `output_tokens` distribution to Table 1**
...
- [ ] **4.4 — Add Kendall's τ for tool sequence ordering**
...
- [ ] **4.5 — Add per-intent one-way ANOVA**
...
- [ ] **4.6 — Add Condition B hallucination deep-dive**
...
- [ ] **4.7 — Add silhouette score computation**
...
- [ ] **4.8 — Add McNemar's test for binary success comparison**
...
- [ ] **4.9 — Stratify all results by model in Tables 1 and 2**
...
- [ ] **4.10 — Re-run `5_analyze_results.py` and regenerate all CSVs**
...
- [ ] **5.1 — Fix Fig 9 (Pareto Frontier) — `Avg_Tokens` computation bug**
...
- [ ] **5.2 — Fix Fig 11 (Success Breakdown) — misleading stacked bars**
...
- [ ] **5.3 — Update Fig 1 and Fig 12 after metric replacement**
...
- [ ] **5.4 — Add Fig: Format Compliance Rate by Condition and Model** *(new)*
...
- [ ] **5.5 — Add Fig: Kendall's τ Tool Ordering Score by Condition** *(new)*
  - Barplot of mean τ ± 95% CI per condition
  - **Why:** Adds the ordering dimension missing from set-based F1

- [ ] **5.6 — Re-run `6_generate_figures.py`** and verify all 13+ figures look correct

---

## 🔵 Group 6 — Documentation Cleanup (Can Run Anytime)

- [ ] **6.1 — Fix Cerebras model name across all documents**
...
- [ ] **6.2 — Remove voice transcription framing from research documents**
...
- [ ] **6.3 — Fix `walkthrough.md` self-contradiction**
...
- [ ] **6.4 — Standardize figure count across all documents**
...
- [ ] **6.6 — Write dataset provenance paragraph for paper**
...
- [ ] Paired t-test p-values (all 3 pairs) computed and verified
- [ ] Cohen's d with 95% CIs computed from `metrics.py` (not inline formula)
- [ ] All 432 rows in `scores` have non-null `intent_fidelity_score`
- [ ] All applicable rows (~180) have non-null `functional_correct`
- [ ] `table1_condition_overall.csv` has ≥10 columns including `format_compliance_rate`, `tool_hallucination_rate`, `intent_fidelity_mean`, `functional_correct_rate`, `mean_output_tokens`, `CPSTC_CI_lower`, `CPSTC_CI_upper`
- [ ] Bootstrap CPSTC CIs computed and documented
- [ ] Per-intent ANOVA table generated (Table 4)
- [ ] Model-stratified Table 1 variant generated
- [ ] All figure bugs fixed; 2 new figures generated
- [ ] `requirements.txt` exists
- [ ] No Cerebras model name inconsistencies remain in any document
- [ ] No voice transcription framing remains in research documents
- [ ] Dataset provenance paragraph written
